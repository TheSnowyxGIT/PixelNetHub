...
import { PackageJson, PackageJsonValidator } from './files/packageJson.validator';
...
const filesToCheck: Record<AppFiles, FileValidator<any>> = {
...
export async function validateApp(appPath: string): Promise<ValidateResult> {
  ...
  let result: ValidateResult = {} as ValidateResult;
  let success = true;
  ...
  logger.log(`App ${colors.white(result['package.json'].name)} at ${colors.white(appPath)} ${colors.green('OK')}`);
  return result;
}
